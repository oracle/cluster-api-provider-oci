...
func TestOCIClusterReconciler_reconcile(t *testing.T) {
	...
	tests := []struct {
		...
		testSpecificSetup  func(cs *mock_scope.MockClusterScopeClient)
	}{
		{
			name:               "all success",
			expectedEvent:      infrastructurev1beta1.FailureDomainEventReady,
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			defer teardown(t, g)
			setup(t, g)
			tc.testSpecificSetup(cs)
			ctx := context.Background()
			_, err := r.reconcile(ctx, cs)
...
func TestOCIClusterReconciler_reconcileDelete(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			tc.testSpecificSetup(cs)
			ctx := context.Background()
			_, err := r.reconcileDelete(ctx, cs)
			actual := conditions.Get(ociCluster, tc.conditionAssertion.conditionType)
