...
func (m *MachineScope) GetOrCreateMachine(ctx context.Context) (*core.Instance, error) {
	...
	m.Logger.Info("Instance is launching", "instanceId", instanceID)
	if instanceID != nil {
		// Wait for instance to be running
		m.Logger.Info("Waiting for instance to be running...")
		...
		for {
			...
			if getInstanceResp.Instance.LifecycleState == core.InstanceLifecycleStateRunning {
				m.Logger.Info("Instance is running")
				break
			}
